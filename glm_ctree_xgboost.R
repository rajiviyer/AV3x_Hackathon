...
ctreemod <- ctree(Disbursed~Gender+City_Type+BWeekday+Lead_Weekday+Lead_Month+Loan_Tenure_Applied+Mobile_Verified+Var5+Var1+Filled_Form+Device_Type+Var4+Age_Class,data=train)
predlogreg <- predict(logreg,type="response",newdata=test)
predctree <- predict(ctreemod,type="prob",newdata=test)
predctree <- sapply(predctree,"[[",2)
...
pred <- 0.1*predlogreg + 0.1*predctree + 0.8*predxgboost
#pred <- 0.3*predlogreg + 0.7*predxgboost
#pred <- rank(predlogreg)/max(rank(predlogreg)) + rank(predxgboost)/max(rank(predxgboost)) + rank(predctree)/max(rank(predctree))
outfile<-data.frame("ID"=testid,Disbursed=pred)
write.csv(outfile,"glm_ctree_xgboost_ensemble.csv",row.names=FALSE)